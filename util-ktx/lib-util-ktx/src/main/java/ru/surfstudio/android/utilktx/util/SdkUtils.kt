...
object SdkUtils {
    ...
    fun runOnLollipop(block: () -> Unit) {
        run(Build.VERSION_CODES.LOLLIPOP, block)
    }
    ...
    fun runOnMarshmallow(block: () -> Unit) {
        run(Build.VERSION_CODES.M, block)
    }
    ...
    fun runOnNoughat(block: () -> Unit) {
        run(Build.VERSION_CODES.N, block)
    }
    ...
    fun runOnOreo(block: () -> Unit) {
        run(Build.VERSION_CODES.O, block)
    }
    ...
    fun runOnPie(block: () -> Unit) {
        run(Build.VERSION_CODES.P, block)
    }
    ...
    fun runOnQ(block: () -> Unit) {
        run(Build.VERSION_CODES.Q, block)
    }
    ...
    fun runOnR(block: () -> Unit) {
        run(Build.VERSION_CODES.R, block)
    }
    ...
    private fun run(sdkLevel: Int, block: () -> Unit) {
        if (Build.VERSION.SDK_INT >= sdkLevel) {
            block()
        }
    }
