...
public class ObservableUtil {
    ...
    /**
     * This function works like {@see Observable#mergeDelayError},
     * but also makes requests parallel using {@param scheduler}
     *
     * @param scheduler Scheduler to use for parallel execution
     * @param sources list of {@see Observable} for merge
     * @param converter function used to convert from T to R
     */
    public static <T, R> Observable<? extends R> mergeDelayError(
        Scheduler scheduler,
        List<Observable<? extends T>> sources,
        FunctionSafe<T, R> converter
    ) {
        List<Observable<? extends R>> newSources = new ArrayList<>();
        ...
        return Observable.mergeDelayError(newSources);
    }

    public static <T1, T2, R> Observable<R> mergeDelayError(
            Scheduler scheduler,
            Observable<T1> o1,
            Observable<T2> o2,
            FunctionSafe<T1, R> converter1,
            FunctionSafe<T2, R> converter2
    ) {
        List<Observable<R>> sources = new ArrayList<>();
        sources.add(o1.map(converter1).subscribeOn(scheduler));
        sources.add(o2.map(converter2).subscribeOn(scheduler));
        return Observable.mergeDelayError(sources);
    }

    public static <T1, T2, T3, R> Observable<R> mergeDelayError(
            Scheduler scheduler,
            Observable<T1> o1,
            Observable<T2> o2,
            Observable<T3> o3,
            FunctionSafe<T1, R> converter1,
            FunctionSafe<T2, R> converter2,
            FunctionSafe<T3, R> converter3
    ) {
        List<Observable<R>> sources = new ArrayList<>();
        sources.add(o1.map(converter1).subscribeOn(scheduler));
        sources.add(o2.map(converter2).subscribeOn(scheduler));
        sources.add(o3.map(converter3).subscribeOn(scheduler));
        return Observable.mergeDelayError(sources);
    }

    public static <T1, T2, T3, T4, R> Observable<R> mergeDelayError(
            Scheduler scheduler,
            Observable<T1> o1,
            Observable<T2> o2,
            Observable<T3> o3,
            ...
            FunctionSafe<T2, R> converter2,
            FunctionSafe<T3, R> converter3,
            FunctionSafe<T4, R> converter4
    ) {
        List<Observable<R>> sources = new ArrayList<>();
        sources.add(o1.map(converter1).subscribeOn(scheduler));
        sources.add(o2.map(converter2).subscribeOn(scheduler));
        sources.add(o3.map(converter3).subscribeOn(scheduler));
        sources.add(o4.map(converter4).subscribeOn(scheduler));
        return Observable.mergeDelayError(sources);
    }

    public static <T1, T2, T3, T4, T5, R> Observable<R> mergeDelayError(
            Scheduler scheduler,
            Observable<T1> o1,
            Observable<T2> o2,
            Observable<T3> o3,
        ...
        sources.add(o3.map(converter3).subscribeOn(scheduler));
        sources.add(o4.map(converter4).subscribeOn(scheduler));
        sources.add(o5.map(converter5).subscribeOn(scheduler));
        return Observable.mergeDelayError(sources);
    }

    public static <T1, T2, T3, T4, T5, T6, R> Observable<R> mergeDelayError(
            Scheduler scheduler,
            Observable<T1> o1,
            Observable<T2> o2,
            Observable<T3> o3,
        ...
        sources.add(o4.map(converter4).subscribeOn(scheduler));
        sources.add(o5.map(converter5).subscribeOn(scheduler));
        sources.add(o6.map(converter6).subscribeOn(scheduler));
        return Observable.mergeDelayError(sources);
    }

    public static <T1, T2, T3, T4, T5, T6, T7, R> Observable<R> mergeDelayError(
            Scheduler scheduler,
            Observable<T1> o1,
            Observable<T2> o2,
            Observable<T3> o3,
        ...
        sources.add(o5.map(converter5).subscribeOn(scheduler));
        sources.add(o6.map(converter6).subscribeOn(scheduler));
        sources.add(o7.map(converter7).subscribeOn(scheduler));
        return Observable.mergeDelayError(sources);
    }

    public static <T1, T2, T3, T4, T5, T6, T7, T8, R> Observable<R> mergeDelayError(
            Scheduler scheduler,
            Observable<T1> o1,
            Observable<T2> o2,
            Observable<T3> o3,
        ...
        sources.add(o6.map(converter6).subscribeOn(scheduler));
        sources.add(o7.map(converter7).subscribeOn(scheduler));
        sources.add(o8.map(converter8).subscribeOn(scheduler));
        return Observable.mergeDelayError(sources);
    }

    public static <T1, T2, T3, T4, T5, T6, T7, T8, T9, R> Observable<R> mergeDelayError(
            Scheduler scheduler,
            Observable<T1> o1,
            Observable<T2> o2,
            Observable<T3> o3,
        ...
        sources.add(o7.map(converter7).subscribeOn(scheduler));
        sources.add(o8.map(converter8).subscribeOn(scheduler));
        sources.add(o9.map(converter9).subscribeOn(scheduler));
        return Observable.mergeDelayError(sources);
    }
}