...
) : BindableItemController<Kitten, KittenItemController.Holder>() {

    override fun getItemId(data: Kitten): String = data.name

    override fun createViewHolder(parent: ViewGroup): Holder = Holder(parent)

    inner class Holder(parent: ViewGroup) : BindableViewHolder<Kitten>(parent, R.layout.list_item_kitties_kitten) {
        ...
        private lateinit var payload: Kitten

        init {
            clickableView.setOnClickListener { onClickedAction(payload) }
        }

        override fun bind(data: Kitten) {
            payload = data
            render()
        }
