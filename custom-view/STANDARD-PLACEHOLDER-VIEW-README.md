...
1) Создать новый класс, реализующий интерфейс `PlaceHolderViewInterface` и расширяющий класс 
`StandardPlaceHolderView`;

2) Реализовать в нём метод `PlaceHolderViewInterface#render(loadState: LoadState)`, который будет 
...
getPlaceHolderView() и вернуть из него экземпляр `PlaceHolderView`.
