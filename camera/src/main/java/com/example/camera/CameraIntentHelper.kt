...
@PerScreen
class CameraIntentHelper(private var activity: Activity, cameraIntentHelperCallback: CameraIntentHelperCallback) {
    companion object {
        internal const val TAKE_PHOTO_CODE = 612
    }
     ...
     */
    private lateinit var dateCameraIntentStarted: Date
    /**
    ...
    /**
     * Potential 3rd location of photo
     */
    ...
    private var cameraIntentHelperCallback: CameraIntentHelperCallback = cameraIntentHelperCallback
    ...
    fun startCameraIntent() {
        if (Environment.getExternalStorageState() == Environment.MEDIA_MOUNTED) {
            try {
                ...
                // 2.) the photo will be stored in two locations (the given localPath and, additionally, in the MediaStore)
        ...
        if (resultCode == Activity.RESULT_OK) {
            ...
            try {
                ...
                if (!myCursor.isAfterLast) {
                    // This will actually give you the file localPath location of the image.
                    ...
                    if (myCursor.moveToNext() && !myCursor.isAfterLast) {
                        ...
                        if (dateOfPicture3rdLocation.after(dateCameraIntentStarted)) {
    ...
    private fun deletePhotoInOtherLocations() {
        if (preDefinedCameraUri != null && preDefinedCameraUri != photoUri) {
            cameraIntentHelperCallback.deletePhotoWithUri(preDefinedCameraUri!!)
        }
        if (photoUriIn3rdLocation != null && photoUriIn3rdLocation != photoUri) {
